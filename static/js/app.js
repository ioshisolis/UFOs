...
function updateFilters() {
    ...
    let contry = d3.select("#country").property("value");
    let shape = d3.select("#shape").property("value");

    // 4b. Save the value that was changed as a variable.
    let inputDate = date.on("change")
    let inputCity = d3.select("#input-field");
    let inputState = d3.select("#input-field");
    let inputCountry= d3.select("#input-field");
    let inputShape = d3.select("#input-field");
